...
# from resnet_model import *
from .BA import SPBlock_ as SPBlock_, PyramidPooling
...
class BASNet(nn.Module):
    def __init__(self, basic_channels=64):
        ...
        self.squeeze2 = nn.Sequential(
            nn.Conv2d(basic_channels*4, basic_channels, 1), nn.BatchNorm2d(basic_channels), nn.ReLU(inplace=True))
        self.squeeze1 = nn.Sequential(
            nn.Conv2d(basic_channels, basic_channels, 1), nn.BatchNorm2d(basic_channels), nn.ReLU(inplace=True))

        # self.refunet = RefUnet(1, 64)

        self.spblock1 = SPBlock_(basic_channels, basic_channels)  # , nn.BatchNorm2d)
        self.spblock12 = SPBlock_(basic_channels, basic_channels)  # , nn.BatchNorm2d)
        self.spblock2 = SPBlock_(basic_channels, basic_channels)  # , nn.BatchNorm2d)
        self.spblock22 = SPBlock_(basic_channels, basic_channels)  # , nn.BatchNorm2d)
        self.spblock3 = SPBlock_(basic_channels*2, basic_channels*2)  # , nn.BatchNorm2d)
        self.spblock32 = SPBlock_(basic_channels, basic_channels)  # , nn.BatchNorm2d)
        self.spblock4 = SPBlock_(basic_channels*2, basic_channels*2)  # , nn.BatchNorm2d)
        self.spblock42 = SPBlock_(basic_channels*2, basic_channels*2)  # , nn.BatchNorm2d)
        self.spblock5 = SPBlock_(basic_channels*4, basic_channels*4)  # , nn.BatchNorm2d)
        self.spblock52 = SPBlock_(basic_channels*2, basic_channels*2)  # , nn.BatchNorm2d)

        self.mpm1 = BasicBlock(basic_channels*4, basic_channels*4)
        ...
        self.mpm12 = BasicBlock(basic_channels*4, basic_channels*4)
        ...
        self.conv1 = nn.Conv2d(basic_channels*4*3, basic_channels*2, 3, padding=1)
        self.bn1 = nn.BatchNorm2d(basic_channels*2)
        self.conv2 = nn.Conv2d(basic_channels*2*3, basic_channels*2, 3, padding=1)
        self.bn2 = nn.BatchNorm2d(basic_channels*2)
        self.conv3 = nn.Conv2d(basic_channels*2*3, basic_channels, 3, padding=1)
        self.bn3 = nn.BatchNorm2d(basic_channels)
        self.conv4 = nn.Conv2d(basic_channels*1*3, basic_channels, 3, padding=1)
        self.bn4 = nn.BatchNorm2d(basic_channels)
        self.conv5 = nn.Conv2d(basic_channels*1*3, basic_channels, 3, padding=1)
        ...
        self.conv_final = nn.Sequential(
            nn.Conv2d(basic_channels, basic_channels, 3, padding=1), nn.BatchNorm2d(
                basic_channels), nn.ReLU(inplace=True),
            nn.Conv2d(basic_channels, 1, 3, padding=1))

        self.spp5 = PyramidPooling(basic_channels*4, nn.BatchNorm2d)
        self.spp4 = PyramidPooling(basic_channels*2, nn.BatchNorm2d)
        self.spp3 = PyramidPooling(basic_channels*2, nn.BatchNorm2d)
        self.spp2 = PyramidPooling(basic_channels, nn.BatchNorm2d)
        self.spp1 = PyramidPooling(basic_channels, nn.BatchNorm2d)
        self.spp5d = PyramidPooling(basic_channels*4, nn.BatchNorm2d)
        self.spp4d = PyramidPooling(basic_channels*2, nn.BatchNorm2d)
        self.spp3d = PyramidPooling(basic_channels*2, nn.BatchNorm2d)
        self.spp2d = PyramidPooling(basic_channels, nn.BatchNorm2d)
        self.spp1d = PyramidPooling(basic_channels, nn.BatchNorm2d)
...
##########################################decoder##########################################
        dec_32 = self.mpm1(self.spp5(skip_32) * defocus5)
        def_32 = self.mpm12(self.spp5d(defocus5) * skip_32)

        dec_out_32, mask_32 = self.spblock5(skip_32 + defocus5)  # *dec_32
        dec_32 = F.relu(
            self.bn1(self.conv1(torch.cat([dec_32, def_32, dec_out_32], 1))), inplace=True)
        dec_32, mask_322 = self.spblock52(dec_32)

        dec_16 = F.interpolate(dec_32, size=(hsize//16, wsize//16),
                               mode="bilinear", align_corners=False)

        # skip_16 = self.spade4(dec_16, skip_16)
        skip_16 = skip_16 + dec_16
        dec_16 = self.mpm2(self.spp4(skip_16) * defocus4)
        def_16 = self.mpm22(self.spp4d(defocus4) * skip_16)

        dec_out_16, mask_16 = self.spblock4(skip_16 + defocus4)  # *dec_16
        dec_16 = F.relu(self.bn2(self.conv2(
            torch.cat((dec_16, def_16, dec_out_16), 1))), inplace=True)  # 64 128 128 128
        dec_16, mask_162 = self.spblock42(dec_16)  # *dec_16

        dec_8 = F.interpolate(dec_16, size=(hsize//8, wsize//8),
                              mode="bilinear", align_corners=False)

        skip_8 = skip_8 + dec_8
        dec_8 = self.mpm3(self.spp3(skip_8) * defocus3)
        def_8 = self.mpm32(self.spp3d(defocus3) * skip_8)

        dec_out_8, mask_8 = self.spblock3(skip_8 + defocus3)  # *dec_8
        dec_8 = F.relu(self.bn3(self.conv3(
            torch.cat((dec_8, def_8,dec_out_8), 1))),inplace=True)  # 64 64 64 64
        dec_8, mask_82 = self.spblock32(dec_8)  # *dec_8

        dec_4 = F.interpolate(dec_8, size=(hsize//4, wsize//4),
                              mode="bilinear", align_corners=False)

        skip_4 = skip_4 + dec_4
        dec_4 = self.mpm4(self.spp2(skip_4) * defocus2)
        def_4 = self.mpm42(self.spp2d(defocus2) * skip_4)

        dec_out_4, mask_4 = self.spblock2(skip_4 + defocus2)  # *dec_4
        dec_4 = F.relu(self.bn4(self.conv4(
            torch.cat((dec_4, def_4,dec_out_4), 1))), inplace=True)
        dec_4, mask_42 = self.spblock22(dec_4)  # *dec_4

        dec_2 = F.interpolate(dec_4, size=(hsize//2, wsize//2),
                              mode="bilinear", align_corners=False)

        skip_2 = skip_2 + dec_2
        dec_2 = self.mpm5(self.spp1(skip_2) * defocus1)
        def_2 = self.mpm52(self.spp1d(defocus1) * skip_2)

        out_out_2, mask_2 = self.spblock1(skip_2 + defocus1)
        dec_2 = F.relu(self.bn5(self.conv5(
            torch.cat((dec_2, def_2, out_out_2), 1))), inplace=True)
        dec_2, mask_2 = self.spblock12(dec_2)
...
# dec_1.shape
# )
        # feature_list = [before_2, mask_2, after_2]
...
if __name__ == "__main__":
    net = BASNet(64).cuda()
    # print(net)
    a, b, c, d, e, f = net(torch.ones(14, 3, 224, 224).cuda(),
                           torch.ones(14, 1, 224, 224).cuda())
