...
class SPADE2(nn.Module):
    def __init__(self, param_free_norm_type, norm_nc, label_nc):
        ...
        self.mlp_shared2 = nn.Sequential(
            nn.Conv2d(label_nc, nhidden, kernel_size=ks, padding=pw),
            nn.ReLU())
        self.mlp_gamma2 = nn.Conv2d(nhidden, norm_nc, kernel_size=ks, padding=pw)
        self.mlp_beta2 = nn.Conv2d(nhidden, norm_nc, kernel_size=ks, padding=pw)

        self.gamma_attn = nn.Sequential(
            nn.Conv2d(2* norm_nc, norm_nc, kernel_size=1, padding=0),
            nn.BatchNorm2d(norm_nc),
    ...
    def forward(self, x, segmap):
        ...
        x_mask = self.attn_RGB(x)
        # Part 2. produce scaling and bias conditioned on semantic map
        # segmap = F.interpolate(segmap, size=x.size()[2:], mode='bilinear')
        actv1 = self.mlp_shared(segmap) *x_mask
        gamma1 = self.mlp_gamma(actv1)
        beta1 = self.mlp_beta(actv1)

        actv2 = self.mlp_shared2(1 - segmap) *x_mask
        gamma2 = self.mlp_gamma2(actv2)
        beta2 = self.mlp_beta2(actv2)

        gamma = self.gamma_attn(torch.cat((gamma1, gamma2),1))
        beta = self.beta_attn(torch.cat((beta1, beta2),1))
...
class BASNet(nn.Module):
    def __init__(self, basic_channels=64):
        ...
        self.spade1 = SPADE2('batch', basic_channels, basic_channels)
        self.spade2 = SPADE2('batch', basic_channels*2, basic_channels*2)
        self.spade3 = SPADE2('batch', basic_channels*2, basic_channels*2)
        self.spade4 = SPADE2('batch', basic_channels*2, basic_channels*2)
        self.spade5 = SPADE2('batch', basic_channels*4, basic_channels*4)
