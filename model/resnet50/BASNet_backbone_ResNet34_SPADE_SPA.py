...
class DefocusNet(nn.Module):
    def __init__(self, n_channels, basic_channels):
        ...
        self.encoder2 = BasicBlock(basic_channels, basic_channels, 2, nn.Sequential(
            nn.Conv2d(basic_channels, basic_channels,
                      kernel_size=(1, 1), stride=(2, 2), bias=False),
            nn.BatchNorm2d(basic_channels, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)))  # 64, 512, 512
        # stage 2
        self.encoder3 = BasicBlock(basic_channels, basic_channels*2, 2, downsample=nn.Sequential(
            nn.Conv2d(basic_channels, basic_channels*2,
                      kernel_size=(1, 1), stride=(2, 2), bias=False),
            nn.BatchNorm2d(basic_channels*2, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)))  # 128, 256, 256
        # stage 3
        self.encoder4 = BasicBlock(basic_channels*2, basic_channels*4, 2, downsample=nn.Sequential(
            nn.Conv2d(basic_channels*2, basic_channels*4,
                      kernel_size=(1, 1), stride=(2, 2), bias=False),
            nn.BatchNorm2d(basic_channels*4, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)))  # 256, 128, 128
        # stage 4
        self.encoder5 = BasicBlock(basic_channels*4, basic_channels*8, 2, downsample=nn.Sequential(
...
class BASNet(nn.Module):
    def __init__(self, basic_channels=64):
        ...
        del net
        self.squeeze5 = nn.Sequential(
            nn.Conv2d(basic_channels*8, basic_channels*4, 1), nn.BatchNorm2d(basic_channels*4), nn.ReLU(inplace=True))
        self.squeeze4 = nn.Sequential(
        ...
        self.spblock1 = SPBlock_attention(basic_channels, basic_channels, nn.BatchNorm2d)
        self.spblock2 = SPBlock_attention(basic_channels, basic_channels, nn.BatchNorm2d)
        self.spblock3 = SPBlock_attention(basic_channels, basic_channels, nn.BatchNorm2d)
        self.spblock4 = SPBlock_attention(basic_channels*2, basic_channels*2, nn.BatchNorm2d)
        self.spblock5 = SPBlock_attention(basic_channels*2, basic_channels*2, nn.BatchNorm2d)
        ...
        self.mpm3 = BasicBlock(basic_channels, basic_channels)
        ...
        self.mpm32 = BasicBlock(basic_channels, basic_channels)
        ...
        self.conv3 = nn.Conv2d(basic_channels*4, basic_channels, 3, padding=1)
        self.bn3 = nn.BatchNorm2d(basic_channels)
        self.conv4 = nn.Conv2d(basic_channels*2, basic_channels, 3, padding=1)
...
##########################################decoder##########################################
        ...
        dec_32 = self.spblock5(dec_32) + dec_32
        ...
        dec_16 = self.spblock4(dec_16) + dec_16
        ...
        dec_8 = self.spblock3(dec_8) + dec_8
        ...
        dec_4 = self.spblock2(dec_4) + dec_4
        ...
        dec_2 = self.spblock1(dec_2) + dec_2
...
if __name__ == "__main__":
    net=BASNet(64).cuda()
    # print(net)
    a,b,c,d,e = net(torch.ones(14, 3, 224, 224).cuda(), torch.ones(14, 1, 224, 224).cuda())
