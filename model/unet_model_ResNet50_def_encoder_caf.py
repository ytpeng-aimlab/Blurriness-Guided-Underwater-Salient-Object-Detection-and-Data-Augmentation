...
class UNet(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True):
        ...
        self.se1 = SKConv3(64)
        self.se2 = SKConv3(256)
        self.se3 = SKConv3(512)
        self.se4 = SKConv3(1024)
        self.se5 = SKConv3(2048)
        ## -------------Side Output--------------
    ...
    def forward(self, x, defocus):
        ...
        # x1 = self.inconv(x)+x1_def
        x1 = self.inconv(x)
        # x1 = self.se1(x1, x1)#x1_def)
        x1 = self.se1(x1, x1_def)
        # print('x1',x1.shape)#x1 torch.Size([3, 64, 256, 256])

        # x2= self.layer1(x1)+x2_def
        x2 = self.layer1(x1)
        # x2= self.se2(x2, x2)#x2_def)
        x2= self.se2(x2, x2_def)
        # print('x2',x2.shape)#x2 torch.Size([3, 256, 128, 128])

        # x3= self.layer2(x2)+x3_def
        x3 = self.layer2(x2)
        # x3= self.se3(x3, x3)#x3_def)
        x3= self.se3(x3, x3_def)
        # print('x3',x3.shape)#x3 torch.Size([3, 512, 64, 64])

        # x4 = self.layer3(x3)+ x4_def
        x4 = self.layer3(x3)
        # x4 = self.se4(x4, x4)#x4_def)
        x4 = self.se4(x4, x4_def)
        # print('x4',x4.shape)#x4 torch.Size([3, 1024, 32, 32])

        # x5 = self.layer4(x4) + x5_def
        x5 = self.layer4(x4)
        # x5 = self.se5(x5, x5)#x5_def)
        x5 = self.se5(x5, x5_def)
